...
func TestTypeCheckMemPackage(t *testing.T) {
	...
	for _, tc := range tt {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := TypeCheckMemPackage(tc.pkg, tc.getter, false, ParseModeAll)
			if tc.check == nil {
...
`
	...
	_, _, _, _, _, err := TypeCheckMemPackage(pkg, mpkgGetter, false, ParseModeAll)
	assert.NoError(t, err)
	...
	`
		...
		_, _, _, _, _, _, err = TypeCheckMemPackage(pkg, mpkgGetter, false, ParseModeAll)
		assert.NoError(t, err)
