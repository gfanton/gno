...
type lazyPackage struct {
	update       int
	parentsDeps  map[string]*lazyPackage
	childrenDeps map[string]*lazyPackage
...
type LazyLoader struct {
	...
	pkgs   map[string]*lazyPackage
}

func (l *LazyLoader) AddPackage(path string) {
	return
}

func (l *LazyLoader) LoadPackages() []*PackagePath {
	return nil
}

func (l *LazyLoader) Invalidate(path string) []*PackagePath {
	return nil
}
...
func (l *LazyLoader) getPkg(path string) *lazyPackage {
	lp, ok := l.pkgs[path]
	if !ok {
		lp := &lazyPackage{
			parentsDeps:  map[string]*lazyPackage{},
...
func (l *LazyLoader) Resolve(path string) {
	return
}

func (l *LazyLoader) sortList() []*std.MemPackage {
	return nil
}

func (l *LazyLoader) loadPackage(path string) (*lazyPackage, error) {
